...
def loadModel():
    ...
    print(getcwd()+"/cars4sale/ml-model/yolov3.weights")
    NN = cv2.dnn.readNet(getcwd()+"/cars4sale/ml-model/yolov3.weights", getcwd()+"/cars4sale/ml-model/yolov3.cfg")
    targetClasses = []
    with open(getcwd()+"/cars4sale/ml-model/coco.names") as f:
...
def predict(files):
    """ Takes Flask file list as input """
    if len(files)!=1:
    ...
    # Convert the PIL Image to one recognized by open cv
    img = np.array(image) 
