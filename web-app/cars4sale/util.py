...
def gen_car_years(gap):
    year = datetime.datetime.today().year
    years = list(range(year, year - gap, -1))
    return years

def get_seats_integer(seat_choice):
    if seat_choice == "Just Me":
        return 1
    elif seat_choice == "More":
        return 8
    else:
        return int(seat_choice)

def get_lowest_age_allowed(age_choice):
    #"New (1-2 yrs)", "Not Too Old (2-5 yrs)", "Older (5-10 yrs)", "Even Older (10-20+ yrs)"
    year = datetime.datetime.now().year
    if age_choice == "New (1-2 yrs)":
        return year - 2
    elif age_choice == "Not Too Old (2-5 yrs)":
        return year - 5
    elif age_choice == "Older (5-10 yrs)":
        return year - 10
    elif age_choice == "Even Older (10-20 yrs)":
        return 1950
...
def to_img64(file, size):
    ...
    ascii = img64.decode("ascii")
    return ascii

def build_image_string(files):
    images = ""
    for f in files:
        images += to_img64(f, 300) + "@"
    return images[:-1]

def get_max_adverts_for_member(membership):
    if membership == None:
        return 1
    if membership == "Basic":
        return 2
...
def get_member_value(membership):
    if membership == None:
        return 0
